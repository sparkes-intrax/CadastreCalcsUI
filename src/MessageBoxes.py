...
class CloseDetectedMessage(QtWidgets.QMessageBox):
    def __init__(self, CloseCheck):
        ...
        self.setText("A Traverse Close was detected for the calculated point:\n\n")
        self.setInformativeText(msgStr)
        self.setWindowTitle("Traverse Close Detected")
        self.addButton('Close && Adjust', QtWidgets.QMessageBox.YesRole)
        self.addButton('Close && NO Adjust', QtWidgets.QMessageBox.AcceptRole)
        self.addButton('NO Close', QtWidgets.QMessageBox.NoRole)
        self.ButVal = self.buttonClicked.connect(self.OnClicked)
        messageBoxFormat(self)
        self.retVal = self.exec_()

    def OnClicked(self, button):
        self.Role = button.text()
...
def CloseDetectedMessage(CloseCheck):
    ...
    #CloseAdjust = QtWidgets.QAbstractButton()
    #CloseAdjust.setText("Close & Adjust")
    #msg.CloseAdjust.set
    msg.addButton('Close && Adjust', QtWidgets.QMessageBox.YesRole)
    msg.addButton('Close && NO Adjust', QtWidgets.QMessageBox.AcceptRole)
    msg.addButton('NO Close', QtWidgets.QMessageBox.NoRole)
    msg.buttonClicked.connect()
    #CloseAdjust = msg.button(QtWidgets.QMessageBox.YesAll)
    #msg.addButton(msg, 'Close & No Adjust', QtWidgets.QMessageBox.Close)
    #msg.addButton(msg, 'No Close', QtWidgets.QMessageBox.NoAll)
    msg = messageBoxFormat(msg)
    returnValue = msg.exec_()

    print(returnValue)
    if returnValue == QtWidgets.QMessageBox.YesRole:
        print("Yes Role")
    elif returnValue == QtWidgets.QMessageBox.AcceptRole:
        print("Accept Role")
    elif returnValue == QtWidgets.QMessageBox.NoRole:
        print("No Role")
    return returnValue
'''
...
def genericMessageYesNo(mes, title):
    msg = QtWidgets.QMessageBox()
    msg.setIcon(QtWidgets.QMessageBox.Warning)
    msg.setText(mes)
    #msg.setInformativeText(error)
